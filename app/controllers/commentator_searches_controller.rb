class CommentatorSearchesController < ApplicationController
  ...
  def name
    ...
    @commentators = Commentator.name_search(name_params).includes([playings: :game], [:popular_movies, :movie_style])
  end

  def play_style
    @commentators = Commentator.play_style_search(play_style_params).includes([playings: :game], [:popular_movies, :movie_style])
  end

  def game_genre
    genre_name = params[:child_genre_name].nil? ? params[:game_genre_search][:parent_genre_name] : params[:child_genre_name]

    @commentators = Commentator.searches_game_genre_search(genre_name).includes([playings: :game], [:popular_movies, :movie_style])
  end
